...
# تنظیمات صفحه و استایل راست‌چین برای ظاهر فارسی شیک
...
if uploaded_master and uploaded_attendance:
    ...
    try:
        ...
        for r1, r2 in zip(row1, row2):
            ...
            if r1_clean and r2_clean:
                # ادغام لایه بالایی (اطلاعات تردد) با زیرمجموعه‌ها (ورود / خروج / گیت)
        ...
        # یکسان‌سازی ستون‌های کلیدی به نام‌های استاندارد جهت پردازش راحت‌تر و دقیق‌تر
        ...
        # اعمال متد Forward Fill برای پر کردن ردیف‌های خالی روزهای بعدی هر پرسنل
    ...
    if st.button("پردازش و استخراج گزارش نهایی ⚙️"):
        ...
        # مرتب‌سازی و فیلتر ستون‌های خروجی نهایی براساس فرمت درخواستی شما
        ...
        with pd.ExcelWriter(buffer_all, engine='xlsxwriter') as writer:
            ...
            # اصلاح اصولی و بدون خطای ویژگی راست‌چین در xlsxwriter با پنداس
            ...
            if 'واحد_سازمانی' in df_output.columns:
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    for dept in unique_departments:
                        ...
                        sheet_name_clean = str(dept)[:31] # محدودیت ۳۱ کاراکتر نام شیت در اکسل
                        
                        with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
                            df_dept.to_excel(writer, index=False, sheet_name=sheet_name_clean)
                            # اصلاح اصولی و بدون خطای ویژگی راست‌چین برای فایل‌های تفکیک‌شده
                            writer.sheets[sheet_name_clean].right_to_left()
                        excel_buffer.seek(0)
                        
                        # تمیزکاری نام فایل برای جلوگیری از ارور سیستم‌عامل در یونیکدها
