...
st.markdown("""
    <div style="text-align: right; direction: rtl;">
        ...
        <p>این نسخه با حل ریشه‌ای مشکل ستون‌های هم‌نام (Duplicate Columns) در ساختار خروجی راهکاران، دیتای کاملاً یکتا و بدون خطا تولید می‌کند.</p>
    </div>
...
if uploaded_master and uploaded_attendance:
    ...
    try:
        ...
        # گام اول: نگاشت اولیه کلمات کلیدی
        for i in range(len(row1)):
            ...
            if "کدپرسنلی" in r1_clean or "کد_پرسنلی" in r1_clean or "کدپرسنلی" in r2_clean:
                raw_headers.append("کدپرسنلی")
            elif "نام و نام خانوادگی" in r1_clean or "نام و نام خانوادگی" in r2_clean:
                raw_headers.append("نام و نام خانوادگی")
            elif "تاریخ" in r1_clean or "تاریخ" in r2_clean:
                raw_headers.append("تاریخ")
            elif "روز" in r1_clean or "روز" in r2_clean:
                raw_headers.append("روز")
            elif "ورود" in r2_clean or "ورود" in r1_clean:
                raw_headers.append("ورود")
            elif "خروج" in r2_clean or "خروج" in r1_clean:
                raw_headers.append("خروج")
            elif "گیت" in r2_clean or "گیت" in r1_clean:
                raw_headers.append("گیت")
            elif r2_clean:
                raw_headers.append(r2_clean)
            elif r1_clean:
                raw_headers.append(r1_clean)
            else:
                raw_headers.append(f"ستون_کمکی_{i}")

        # گام دوم: منطق کلیدی یکتا‌سازی ستون‌های تکراری (Deduplication)
        combined_headers = []
        counts = {}
        for item in raw_headers:
            if item in counts:
                counts[item] += 1
                # اگر ستون تکراری بود، شناسه عددی یکتا به آن اضافه می‌کنیم
                combined_headers.append(f"{item}_{counts[item]}")
            else:
                counts[item] = 1
                combined_headers.append(item)
        ...
        # تخصیص دقیق هدرهای کاملاً یکتا به دیتافریم
        df_attendance.columns = combined_headers[:len(df_attendance.columns)]
        
        # استانداردسازی دسترسی به ستون‌های اصلی (حتی اگر تکراری یا شماره‌دار شده باشند)
        # برای نمونه ستون اولِ ورود یا گیت را به عنوان ستون مبنا نگه می‌داریم
        rename_dict = {}
        for col in df_attendance.columns:
            if col.startswith('ورود') and col != 'ورود': rename_dict[col] = 'ورود'
            if col.startswith('خروج') and col != 'خروج': rename_dict[col] = 'خروج'
            if col.startswith('گیت') and col != 'گیت': rename_dict[col] = 'گیت'
            if col.startswith('تاریخ') and col != 'تاریخ': rename_dict[col] = 'تاریخ'
            if col.startswith('روز') and col != 'روز': rename_dict[col] = 'روز'
        
        # اما برای اعمال در دیتافریم، باید مطمئن شویم تداخل مجدد ایجاد نمی‌شود، پس فقط ستون‌های اصلی بدون پسوند را مپ می‌کنیم
        # اگر ستون اصلی بدون پسوند وجود نداشت، اولین نسخه شماره‌دار را به ستون اصلی تغییر نام می‌دهیم
        for base_name in ['تاریخ', 'روز', 'ورود', 'خروج', 'گیت']:
            if base_name not in df_attendance.columns:
                target_dup = f"{base_name}_2"
                if target_dup in df_attendance.columns:
                    df_attendance.rename(columns={target_dup: base_name}, inplace=True)
    ...
    st.success("ساختار چندسطحی فایل تردد تفکیک و نام‌های هم‌نام با موفقیت یکتا شدند.")
    st.divider()
    ...
    if st.button("پردازش و استخراج گزارش نهایی ⚙️"):
        ...
        # لیست ایده‌آل ستون‌های خروجی نهایی بدون تکرار کدهای سیستمی
        desired_columns = ['کدپرسنلی', 'نام و نام خانوادگی', 'تاریخ', 'روز', 'ورود', 'خروج', 'گیت', 'واحد_سازمانی', 'انتقالی_به', 'زیر_مجموعه']
        
        # انتخاب ستون‌های موجود و مرتب‌سازی نهایی و حذف هرگونه نام تکراری احتمالی در بدنه نهایی
        final_cols_to_display = []
        for c in desired_columns:
            if c in df_final.columns and c not in final_cols_to_display:
                final_cols_to_display.append(c)
        
        df_output = df_final[final_cols_to_display].copy()
        ...
        st.markdown("<div style='text-align: right;'><h4>پیش‌نمایش گزارش جامع (اطلاعات یکتا شده ورود، خروج، گیت و ساختار سازمانی)</h4></div>", unsafe_allow_html=True)
